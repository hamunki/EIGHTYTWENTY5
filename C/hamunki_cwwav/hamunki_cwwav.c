...
void text_to_morse(FILE *f) {
	int space = 0;
	/**2023-10-13
	save the spaces and fspaces in a buffer
	and send them after the char .

	Try this: Anytime there is a call to send_space or send_fspace
	put that value in the buffer and send the buffer at the end then 
	call send_char
	*/
	//int space_buffer = 0;
	//int fspace_buffer = 0;
	...
	while ((c = fgetwc(f)) != WEOF) {
		...
		if (c == 13)
			continue;
		// Increment the newline counter and the space counter
		if (c == 10) {
			nl++;
			space++;
			continue;
		}
		// Check for wide whitespace
		if (iswspace(c)) {
			space++;
			continue;
		}
		// If newline is greater than one reset space
		// 2 is a paragraph, more is a section break
		if (nl > 1) {
			space=0;
			//space_buffer = 0;
			if (nl == 2) {	
		...
		//send_fspace(fspace_buffer);
		//send_space(space_buffer);
		space=0;
		//send_char(c);
		//printf("\nAbout to output char [s:%d] [fs:%d]\n\n", space_buffer,  fspace_buffer);
		send_char(c);
	}
	dprintf("C: %d %m\n", c);
}
